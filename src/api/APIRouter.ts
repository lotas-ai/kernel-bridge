...
export class APIRouter {
  ...
  private setupRoutes(): void {
    ...
    this.router.get('/sessions/:session_id', this.getSession.bind(this));
    this.router.delete('/sessions/:session_id', this.deleteSession.bind(this));
    this.router.post('/sessions/:session_id/restart', this.restartSession.bind(this));
    this.router.post('/sessions/:session_id/interrupt', this.interruptSession.bind(this));
    this.router.post('/sessions/:session_id/kill', this.killSession.bind(this));
    this.router.post('/sessions/:session_id/start', this.startSession.bind(this));
    this.router.put('/sessions/:session_id/adopt', this.adoptSession.bind(this));
    this.router.get('/sessions/:session_id/channels', this.getChannels.bind(this));
    this.router.get('/sessions/:session_id/connection_info', this.getConnectionInfo.bind(this));
  }
  ...
  private async clientHeartbeat(req: Request, res: Response): Promise<void> {
    try {
      const { process_id }: ClientHeartbeatRequest = req.body;
      
      if (!process_id || typeof process_id !== 'number') {
        this.sendError(res, 400, 'Invalid Request', 'process_id is required and must be a number');
        return;
      }

      // Log the heartbeat (kernelBridge doesn't require any specific response)
      console.log(`Client heartbeat received from process ${process_id}`);
  ...
  private async setServerConfiguration(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (config.idle_shutdown_hours !== undefined) {
        console.log(`Server idle shutdown configured to ${config.idle_shutdown_hours} hours`);
      }
  ...
  private async createSession(req: Request, res: Response): Promise<void> {
    console.log(`[API] CREATE_SESSION REQUEST body:`, JSON.stringify(req.body, null, 2));
    console.log(`[API] CREATE_SESSION REQUEST headers:`, JSON.stringify(req.headers, null, 2));
    ...
    try {
      ...
      console.log(`[API] CREATE_SESSION: isKernelBridgeRequest = ${isKernelBridgeRequest}`);
      ...
      // Create session
      console.log(`[API] CREATE_SESSION: Calling SessionManager.createSession with sessionId = ${explicit_session_id || 'auto-generated'}`);
      ...
      console.log(`[API] CREATE_SESSION: Session created successfully with ID: ${session.sessionId}, state: ${session.state}`);
      ...
      console.log(`[API] CREATE_SESSION RESPONSE:`, JSON.stringify(response, null, 2));
      res.status(201).json(response);

    } catch (error) {
      console.log(`[API] CREATE_SESSION FATAL ERROR:`, error);
  ...
  private async startSession(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!session) {
        console.log(`[API] START_SESSION ERROR: Session ${sessionId} not found`);
        this.sendError(res, 404, 'Session Not Found', `Session ${sessionId} not found`);
        return;
      }

      console.log(`[API] START_SESSION: Found session ${sessionId} in state: ${session.state}`);
      ...
      if (session.state === 'starting' || session.state === 'idle') {
        ...
        try {
          console.log(`[API] START_SESSION: Getting kernel info for session ${sessionId}`);
          ...
          if (kernelInfo) {
            ...
            console.log(`[API] START_SESSION RESPONSE: Returning kernel info:`, JSON.stringify(response, null, 2));
            res.json(response);
          } else {
            ...
            console.log(`[API] START_SESSION RESPONSE: Kernel info not available, returning fallback:`, JSON.stringify(response, null, 2));
            res.json(response);
          }
        } catch (kernelInfoError) {
          console.log(`[API] START_SESSION ERROR: Failed to get kernel info:`, kernelInfoError);
          ...
          console.log(`[API] START_SESSION RESPONSE: Error getting kernel info, returning simple success:`, JSON.stringify(response, null, 2));
          res.json(response);
        }
      } else {
        console.log(`[API] START_SESSION ERROR: Session ${sessionId} in invalid state: ${session.state}`);
        this.sendError(res, 400, 'Invalid State', 'Session cannot be started in current state');
      }
    } catch (error) {
      console.log(`[API] START_SESSION FATAL ERROR:`, error);
  ...
  private async adoptSession(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      console.log(`[API] ADOPT_SESSION: Attempting to adopt session ${sessionId}`);
      
      // Check if session exists
      const session = this.sessionManager.getSession(sessionId);
      if (!session) {
        console.log(`[API] ADOPT_SESSION ERROR: Session ${sessionId} not found`);
        this.sendError(res, 404, 'Session Not Found', `Session ${sessionId} not found`);
        return;
      }

      console.log(`[API] ADOPT_SESSION: Found session ${sessionId} in state: ${session.state}`);

      // Try to get kernel info for proper adoption
      try {
        console.log(`[API] ADOPT_SESSION: Getting kernel info for session ${sessionId}`);
        const kernelInfo = await this.sessionManager.getKernelInfo(sessionId);
        if (kernelInfo) {
          // Return the actual kernel_info_reply structure
          const response = {
            ...kernelInfo,
            status: 'ok'
          };
          console.log(`[API] ADOPT_SESSION RESPONSE: Returning kernel info:`, JSON.stringify(response, null, 2));
          res.json(response);
          return;
        }
      } catch (kernelInfoError) {
        console.log(`[API] ADOPT_SESSION WARNING: Failed to get kernel info:`, kernelInfoError);
      }

      // Fallback to stub response (this is what's causing the issue!)
      const stubResponse = { 
        status: 'ok',
        message: 'Session adoption not fully implemented yet', 
        session_id: sessionId 
      };
      console.log(`[API] ADOPT_SESSION RESPONSE: Returning STUB response (THIS IS THE PROBLEM!):`, JSON.stringify(stubResponse, null, 2));
      res.json(stubResponse);
    } catch (error) {
      console.log(`[API] ADOPT_SESSION FATAL ERROR:`, error);
  ...
  private convertKernelBridgeEnv(kernelBridgeEnv: any[]): { [key: string]: string } {
    ...
    for (const varAction of kernelBridgeEnv) {
      // Handle both 'type' and 'action' fields, and both 'set' and 'replace' values
      const actionType = varAction.type || varAction.action;
      if ((actionType === 'set' || actionType === 'replace') && varAction.name && varAction.value !== undefined) {
  ...
  getEndpoints(): Array<{ method: string; path: string; description: string }> {
    return [
      ...
      { method: 'GET', path: '/sessions/:session_id', description: 'Get detailed information about a specific session' },
      { method: 'GET', path: '/sessions', description: 'List all sessions with summary information' },
      { method: 'POST', path: '/sessions/:session_id/restart', description: 'Restart a kernel session' },
      { method: 'DELETE', path: '/sessions/:session_id', description: 'Terminate a kernel session' },
      { method: 'POST', path: '/sessions/:session_id/interrupt', description: 'Send interrupt signal to kernel' },
      { method: 'POST', path: '/sessions/:session_id/kill', description: 'Kill a kernel session' },
      { method: 'PUT', path: '/sessions/:session_id/adopt', description: 'Adopt existing session' },
      { method: 'GET', path: '/sessions/:session_id/channels', description: 'Get WebSocket channels URL for session' },
      { method: 'GET', path: '/sessions/:session_id/connection_info', description: 'Get connection info' },
