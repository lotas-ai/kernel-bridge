...
export class WebSocketHandler extends EventEmitter {
  ...
  private async handleIncomingMessage(message: any): Promise<void> {
    ...
    try {
      ...
      if (!message || typeof message !== 'object') {
        console.log(`[WS-ERROR] ${this.sessionId} Invalid message: not an object`);
      ...
      if (!message.type && message.header && message.header.msg_type) {
        console.log(`[WS-WRAP] ${this.sessionId} Auto-wrapping raw Jupyter message: ${message.header.msg_type}`);
        ...
        console.log(`[WS-WRAP] ${this.sessionId} Wrapped message:`, JSON.stringify(wrappedMessage, null, 2));
  ...
  send(message: any): boolean {
    console.log(`[WS-OUT] ${this.sessionId} Sending message:`, JSON.stringify(message, null, 2));
    
    if (this.isClosing || this.ws.readyState !== WebSocket.OPEN) {
      console.log(`[WS-ERROR] ${this.sessionId} Cannot send message - WebSocket not open (state: ${this.ws.readyState})`);
    ...
    try {
      ...
      console.log(`[WS-OUT] ${this.sessionId} Message size: ${messageSize} bytes`);
      ...
      console.log(`[WS-OUT] ${this.sessionId} Message sent successfully`);
