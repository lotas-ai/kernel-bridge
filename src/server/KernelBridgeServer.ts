...
export class KernelBridgeServer extends EventEmitter {
  ...
  private setupMiddleware(): void {
    ...
    this.app.use((req: Request, res: Response, next: NextFunction) => {
      ...
      console.log(`[HTTP-IN] ${requestId} ${req.method} ${req.url}`);
      console.log(`[HTTP-IN] ${requestId} Headers:`, JSON.stringify(req.headers, null, 2));
      if (req.body && Object.keys(req.body).length > 0) {
        console.log(`[HTTP-IN] ${requestId} Body:`, JSON.stringify(req.body, null, 2));
      }
      
      // Capture original res.json to log responses
      const originalJson = res.json;
      res.json = function(body: any) {
        console.log(`[HTTP-OUT] ${requestId} ${req.method} ${req.url} Status: ${res.statusCode}`);
        console.log(`[HTTP-OUT] ${requestId} Response:`, JSON.stringify(body, null, 2));
        return originalJson.call(this, body);
      };
      
      res.on('finish', () => {
        const duration = Date.now() - start;
        console.log(`[HTTP-COMPLETE] ${requestId} ${req.method} ${req.url} ${res.statusCode} (${duration}ms)`);
